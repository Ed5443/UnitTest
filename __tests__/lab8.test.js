describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartText = await cartCount.getProperty('innerText');
    expect(cartText['_remoteObject'].value).toBe("20"); 
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const button = await root.$('button');
      const innerText = await button.getProperty('innerText');
      if(innerText['_remoteObject'].value != "Remove from Cart") { remembers = false; }
    }
    const cartCount = await page.$('#cart-count');
    const cartText = await cartCount.getProperty('innerText');
    if(cartText['_remoteObject'].value != "20") { remembers = false; }
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    const cartText = await cartCount.getProperty('innerText');
    expect(cartText['_remoteObject'].value).toBe("0"); 
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const button = await root.$('button');
      const innerText = await button.getProperty('innerText');
      if(innerText['_remoteObject'].value != "Add to Cart") { remembers = false; }
    }
    const cartCount = await page.$('#cart-count');
    const cartText = await cartCount.getProperty('innerText');
    if(cartText['_remoteObject'].value != "0") { remembers = false; }
    expect(remembers).toBe(true);
  }, 10000);
